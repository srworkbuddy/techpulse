...
- **技术动态采集与沉淀**：定时从你订阅的源（GitHub Trending、Hacker News、官方博客、内部 RSS 都行）抓信息进来，AI 按你定的标签分类、去重、打分，攒成日报或简报，推到企微群或者存档到腾讯文档 / 乐享。想加什么源、改什么标签、推给谁，都你自己说了算。
- **多方案选型对比**：扔 2~5 个备选方案进来——技术、供应商、产品、架构都行——按你给的维度（性能、成本、社区、风险……可加）自动拉数据、出雷达图、给加权评分，吐一份选型报告：推谁、有什么坑、POC 怎么走。能直接当 ADR 用。
- **代码库知识资产生成**：喂一个仓库地址（GitHub、GitLab、Gitee、本地都行）给它，扫完目录结构、关键模块、模块之间的依赖关系，吐三样东西：架构图、决策文档、新人 30 分钟上手指南，直接发到团队知识库。下次再扫，还能对比两次的差异。
