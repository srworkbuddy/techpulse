...
- **多源信息搜索**：从外部信息源（GitHub Trending / Hacker News / 官方博客 / 内部 RSS / 企微群消息）和内部知识库（腾讯文档 / 乐享 / Notion / 本地 SQLite）按关键词 / 标签 / 时间窗拉取原始信息流，让 LLM Agent 能像查数据库一样按需调用，外部与内部的边界被协议层抹平。

- **智能整合**：对搜回来的信息做 AI 分类（按你的标签体系）、去重（相似合并）、打分（1~5 星重要性）、关联（拉到知识图谱里的相关条目），结构化输出日报 / 简报 / 选题清单，可直接推到企微群或存档到腾讯文档，无需人工二次整理。

- **知识沉淀**：把整合后的产物（日报里的高星动态、选型报告、架构决策）以及从仓库扫描出来的结构化资产（架构图 / ADR / 新人 30 分钟上手指南）一并写入团队知识库，支持按主题 / 项目 / 时间回看，下次再扫还能对比差异，让一次采集变成可复用的长期资产。
